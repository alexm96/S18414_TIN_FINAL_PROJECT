...
const FullPageAd=require("../models/advertisement").FullPageAd
...
exports.getSpecificAd = async (adId) => {
    // get associated user, then get inline user info , then ad + image data
    const connection = await mysql.createConnection(mysqlConnection);
    try {
        const userQuery = mysql.format("select user_id from user_advertisement where ad_id=?", [adId])
        const [userIdRow, userIdField] = await connection.execute(userQuery)
        const userId = userIdRow[0]["user_id"]
        const userInfoQuery=mysql.format("select * from user where id=?",[userId])
        const [userRow,userField]=await connection.execute(userInfoQuery)
        const userInfoRow=userRow[0]
        const ImageInfoQuery=mysql.format("select image.* from image join advertisement on advertisement.image_id=image.id where advertisement.id=?",[adId])
        const [imageRow,imageField]=await connection.execute(ImageInfoQuery)
        const imageInfoRow=imageRow[0]
        const adQuery=mysql.format("select advertisement.*,location.city,location.postal_code,tag.name as category from advertisement join location on advertisement.id=location.id join tag on tag.id=advertisement.tag_id where advertisement.id=?",[adId])
         const [adRow,adField]=await connection.execute(adQuery)
        const adInfoRow=adRow[0]
        const completeAd= new FullPageAd(userInfoRow,imageInfoRow,adInfoRow)
        return completeAd
    } catch (error) {
        console.log(error);
